    ...
    for (var col in grid[row]) {
      if (is_numeric(grid[row][col]) && !number) {
        ...
        for (var ngb of neighbors) {
          ...
          if (grid[ngb_row][ngb_col] == "*") {
            if (map[ngb]) {
              map[ngb].push(parseInt(grid[row].slice(start, end + 1).join("")));
            } else {
              map[ngb] = [parseInt(grid[row].slice(start, end + 1).join(""))];
            }
            break;
          }
        }
