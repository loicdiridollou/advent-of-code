// Part 2 for day 12 of 2023

import { arrayEquals } from "./part1";

let cache: { [index: string]: number } = {};

function countConfigMemo(cfg: string, nums: number[]): number {
  if (cfg == "") {
    return arrayEquals(nums, []) ? 1 : 0;
  }
  if (arrayEquals(nums, [])) {
    return cfg.includes("#") ? 0 : 1;
  }
  let total = 0;

  let key = `${cfg}__${nums.join(",")}`;
  if (cache[key]) {
    return cache[key];
  }

  if (".?".includes(cfg[0])) {
    // we can treat the ? as a . and see
    total += countConfigMemo(cfg.slice(1), nums);
  }
  if ("#?".includes(cfg[0])) {
    // need to do more checks
    if (
      nums[0] <= cfg.length &&
      !cfg.slice(0, nums[0]).includes(".") &&
      (nums[0] == cfg.length || cfg[nums[0]] != "#")
    ) {
      total += countConfigMemo(cfg.slice(nums[0] + 1), nums.slice(1));
    }
  }

  cache[key] = total;
  return total;
}

const repeat = (arr: any, n: number) => [].concat(...Array(n).fill(arr));

export function part2(input: string): number {
  let dataGroups = input
    .split("\n")
    .filter((group) => group != "")
    .map((group) => group.split(" "));

  let total = [];
  for (let data of dataGroups) {
    let code = data[1].split(",").map((group) => parseInt(group));
    total.push(countConfigMemo(repeat(data[0], 5).join("?"), repeat(code, 5)));
  }

  return total.reduce((x, y) => x + y, 0);
}
